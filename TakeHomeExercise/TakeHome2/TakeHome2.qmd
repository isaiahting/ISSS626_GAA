...
```{r}
pacman::p_load(tidyverse, sf, spatstat, ggplot2, ggmap, tmap, dplyr, lubridate, raster, gtsummary, tidyr, spdep, knitr, plotly)
```
...
```{r}
#| echo: false
basemap = read_rds("data/rds/basemap.rds")
```

```{r}
#| echo: false
write_rds(rev, "data/rds/rev.rds")
```

```{r}
#| echo: false
rev = read_rds("data/rds/rev.rds")
```
...
```{r}
# Assuming your data is in a variable called 'combined'
# If it's not, replace 'combined' with your actual data frame name

# Calculate total revenue for each province and period
revenue_summary <- combined %>%
...
```{r}
moran.test(combined$revenue_foreign,
           listw = rswm_knn8,
           zero.policy = TRUE,
           na.action = na.omit)
```
