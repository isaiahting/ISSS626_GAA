...
```{r}
write.csv(mdata,"data/model/mdata.csv", row.names = FALSE)
```
...
Possible reasons when no regression is fixed, there might be same coordinates with different attributes. To Find duplicate records.

2nd way is to do gitter #gitter displays the coordinates alittle away from each other.

Read on this [article](https://blogs.sas.com/content/iml/2011/07/06/to-jitter-or-not-to-jitter-that-is-the-question.html) more on jittering.
:::
